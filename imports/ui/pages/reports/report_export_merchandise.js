...
const fillData = (sheet, data) => {
  ...
  sheet.getCell("A1").value = "Event Merchandise Report";
  ...
  sheet.getCell("H3").value = "Merchandise Size";
  sheet.getCell("I3").value = "Merchandise Remark";
  ...
  data.forEach((d, index) => {
    ...
    sheet.getCell(`H${baseCount + index}`).value = d.sizePreference;
    sheet.getCell(`I${baseCount + index}`).value = d.sizePreferenceRemark;
  });
  let displaySummary = "";
  ...
  sheet.getCell(
    "A2"
  ).value = `Total Guests: ${data.length} | Merchandise Size: ${displaySummary}`;
