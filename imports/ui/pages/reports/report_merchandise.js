...
import "./report_merchandise.scss";
import "./report_merchandise.html";
import { Guests } from "../../../api/guests/guests.js";
import {
  getTotalGuestCount,
  getTotalGuestBySortDataMerchandise
} from "../../../api/guests/methods.js";
import { guestsPerPage } from "../../../api/guests/guestsPerPage.js";
import exportReport from "./report_export_merchandise.js";
...
Template.report_merchandise.onRendered(function() {
  skipCount.set(0);
  currentPage.set(0);
  searchTerm.set("");
...
Template.report_merchandise.events({
...
Template.report_merchandise.helpers({
  serviceAddPath() {
  ...
  sizeOptions() {
    const getData = Guests.find({ eventId: FlowRouter.getParam("id") }).fetch();
    let sizeOpt = {};
    let result = [];
    getData.forEach(
      el => (sizeOpt[el.sizePreference] = (sizeOpt[el.sizePreference] || 0) + 1)
    );
    delete sizeOpt["undefined"];
    for (let key in sizeOpt) {
      result.push({ type: key, value: sizeOpt[key] });
    }
    return result;
  }
});

Template.report_merchandise_guest_item.helpers({
  PageNumbers(index) {
    return skipCount.get() + index + 1;
  }
});

Template.report_merchandise_guest_list.onRendered(function() {
  this.autorun(() => {
    this.$(".tooltip-icon").tooltip({ delay: 50 });
...
Template.report_merchandise_guest_list.helpers({
...
Template.report_merchandise_guest_list.events({
  ...
  "click .exportReport"(event) {
    getTotalGuestBySortDataMerchandise.call(
      {
        eventId: FlowRouter.getParam("id"),
        sortField: sortField,
...
Template.report_merchandise_guest_item.onCreated(function() {});

Template.report_merchandise_guest_item.onRendered(function() {});

Template.report_merchandise_guest_item.helpers({});

Template.report_merchandise_guest_item.events({
