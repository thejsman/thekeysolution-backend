...
import "./report_meal_pref.scss";
import "./report_meal_pref.html";
import { Guests } from "../../../api/guests/guests.js";
import {
  getTotalGuestCount,
  getTotalGuestBySortDataMeal
} from "../../../api/guests/methods.js";
import { guestsPerPage } from "../../../api/guests/guestsPerPage.js";
import exportReport from "./report_export_meal_pref.js";
...
Template.report_meal_pref.onRendered(function() {
  skipCount.set(0);
  currentPage.set(0);
  searchTerm.set("");
...
Template.report_meal_pref.events({
...
Template.report_meal_pref.helpers({
  serviceAddPath() {
  ...
  mealOptions() {
    ...
    let result = [];
    guestdata.forEach(
      el => (mealOpt[el.foodPreference] = (mealOpt[el.foodPreference] || 0) + 1)
    );
    delete mealOpt["undefined"];
    for (let key in mealOpt) {
      result.push({ type: key, value: mealOpt[key] });
    }
    return result;
  }
});

Template.report_meal_pref_guest_item.helpers({
  PageNumbers(index) {
    return skipCount.get() + index + 1;
  }
});

Template.report_meal_pref_guest_list.onRendered(function() {
  this.autorun(() => {
    this.$(".tooltip-icon").tooltip({ delay: 50 });
...
Template.report_meal_pref_guest_list.helpers({
...
Template.report_meal_pref_guest_list.events({
  ...
  "click .exportReport"(event) {
    getTotalGuestBySortDataMeal.call(
      {
        eventId: FlowRouter.getParam("id"),
        sortField: sortField,
...
Template.report_meal_pref_guest_item.onCreated(function() {});

Template.report_meal_pref_guest_item.onRendered(function() {});

Template.report_meal_pref_guest_item.helpers({});

Template.report_meal_pref_guest_item.events({
