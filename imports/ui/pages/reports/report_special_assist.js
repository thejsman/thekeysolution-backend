...
import "./report_special_assist.scss";
import "./report_special_assist.html";
import { Guests } from "../../../api/guests/guests.js";
import {
  getTotalGuestCount,
  getTotalGuestBySortDataAssistance
} from "../../../api/guests/methods.js";
import { guestsPerPage } from "../../../api/guests/guestsPerPage.js";
import exportReport from "./report_export_special_assist.js";
...
const allGuests = Guests.find({ eventId: "BAzYGvdKAZXQR62Na" }).fetch();
...
Template.report_special_assist.onRendered(function() {
  skipCount.set(0);
  currentPage.set(0);
  searchTerm.set("");
...
Template.report_special_assist.events({
...
Template.report_special_assist.helpers({
  serviceAddPath() {
  ...
  //WIP - Show Special Assistance summary

  // guestSpecialAssistanceOptions() {
  //   const guests = Guests.find({ eventId: FlowRouter.getParam("id") }).fetch();

  //   let sa = guests
  //     .filter(g => g.specialAssistance !== undefined)
  //     .map(g => g.specialAssistance);
  // },
...
Template.report_special_assist_guest_item.helpers({
  PageNumbers(index) {
    return skipCount.get() + index + 1;
  }
});

Template.report_special_assist_guest_list.onRendered(function() {
  this.autorun(() => {
    this.$(".tooltip-icon").tooltip({ delay: 50 });
...
Template.report_special_assist_guest_list.helpers({
...
Template.report_special_assist_guest_list.events({
  ...
  "click .exportReport"(event) {
    ...
    getTotalGuestBySortDataAssistance.call(
      {
        eventId: FlowRouter.getParam("id"),
        sortField: sortField,
...
Template.report_special_assist_guest_item.onCreated(function() {
  // this.downloading = new ReactiveVar(false);
  // this.familyCount = new ReactiveVar(0);
  // this.familyList = new ReactiveVar();
});

Template.report_special_assist_guest_item.onRendered(function() {});

Template.report_special_assist_guest_item.helpers({});

Template.report_special_assist_guest_item.events({
