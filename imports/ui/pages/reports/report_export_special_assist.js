...
const fillData = (sheet, data) => {
  ...
  sheet.getCell("A1").value = "Event Special Assistance Report";
  ...
  sheet.getCell("H3").value = "Special Assistance";
  sheet.getCell("I3").value = "Special Assistance Remark";
  ...
  data.forEach((d, index) => {
    ...
    sheet.getCell(`H${baseCount + index}`).value = d.specialAssistance
      ? d.specialAssistance.join(", ")
      : "";
    sheet.getCell(`I${baseCount + index}`).value = d.specialAssistanceRemark;
  });
...
export default data => {
  ...
  workbook.xlsx.writeBuffer().then(function(data) {
    FileSaver.saveAs(
      ...
      `Guest-Special-Assistance-Report.xlsx`
    );
  });
};
