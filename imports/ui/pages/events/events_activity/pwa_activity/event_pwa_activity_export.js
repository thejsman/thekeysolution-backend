...
import { downloadEventPwaActivityExcel as downloadEventActivityExcel } from '../../../../../api/pwa_activity_record/methods.js';
import XLSX from 'xlsx';
import FileSaver from 'file-saver';
import './event_pwa_activity_export.html';
import './event_pwa_activity_export.scss';
import moment from 'moment';

Template.event_pwa_activity_export.onRendered(function () {
  this.$('.modal').modal();
});
...
Template.event_pwa_activity_export.events({
  'submit #event-export-pwa-activity-form'(event, template) {
    event.preventDefault();
    var jq = template.$(event.target);
    // console.log('jq222', jq);
    var extraInfo = jq.serializeObject();
    let xxx = $('#event-pwa-activity-export-modal').attr('searchdata');
