import './notificationGuestList.html';
...
Template.notificationGuestList.onRendered(function(){
...
Template.notificationGuestList.helpers({
  guestList: () => {
...
async function fetchGuests(type) {
  ...
  const guestList =  await fetchGuestList.call({eventId, type : type}, (err, res) => {
...
Template.notificationGuestList.events({
  'change .selectGuestListType input[type="radio"]' (event, template) {
    event.preventDefault();
    fetchGuests(event.target.value);
  },

  'change .guestListNotification input[type="checkbox"]' (event, template) {
    event.preventDefault();
    let selectGuests;
    if(Session.get('selectedGuestIds')) {
