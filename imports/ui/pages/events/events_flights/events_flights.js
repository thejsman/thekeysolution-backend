...
Template.events_flights.helpers({
  ...
  flightUploadPath() {
    return FlowRouter.path('events.flight.upload', { id: FlowRouter.getParam("id") });
  },
});
...
Template.flight_card.helpers({
  ...
  flightPNR(){
    let flightPNR = this.flight.flightPNRs;
    return flightPNR;
  },
  ...
  flightAirline() {
    let legs = this.flight.flightLegs;
    let flightAirline = Airlines.findOne(legs[0].flightId);
    return flightAirline.airlineName;
  },

  flightClass() {
    let flightClass = this.flight.flightIsBusiness;
    return flightClass ? "Economy" : "Business";
  },
